...
generate: $(ORIGINAL_HEADER) $(GENERATOR_SCRIPT) $(GENERATOR_SCRIPT_FILES)
	$(RUBY) $(GENERATOR_SCRIPT) $(GENERATOR_MODE) $(GENERATOR_SPECIAL_FLAGS) $(GENERATOR_NO_FORWARDING_TO_MOCK) $(GENERATOR_FILTER) $(GENERATOR_SOURCES) $(GENERATOR_OUTPUT_HEADER) $(GENERATOR_FILL_VTABLE) $< $(LINK_ERROR_LOG) $(GENERATED_FILES) $(CLANG_FLAGS) $(GENERATOR_FLAGS)
	$(LS) ./$(GENERATED_FILE_DIR)/*_Stub.hpp
	$(LS) ./$(GENERATED_FILE_DIR)/*_1.hpp
	$(LS) $(GENERATED_FILE_DIR)/mock_$(ORIGINAL_HEADER_BASENAME)_1.hpp
	$(GREP) -q $(GENERATOR_MARK_IN_MOCK_CPP_FILE) $(GENERATED_FILE_DIR)/*.hpp; test $$? -eq 1
	$(foreach file,$(wildcard $(GENERATED_FILE_DIR)/*.cpp),$(GREP) -q $(GENERATOR_MARK_IN_MOCK_CPP_FILE) $(file);)
